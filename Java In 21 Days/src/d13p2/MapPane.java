	...
	public void paintComponent(Graphics comp) {
		...
		for (int ax = 0; ax < 340; ax += 10) {
			for (int ay = 0; ay < 340; ay += 10) {
				// Each wave occupies 10x10 space.  
				Arc2D.Float wave = new Arc2D.Float(
						ax, ay, 10, 10, 0, -180, Arc2D.OPEN);
				comp2D.draw(wave);
			}
		}
		// Draw Florida, which is a polygon.  
		// Cyclic gradient fill.  
		GradientPaint gp = new GradientPaint(
				0F, 0F, Color.green, 350F, 350F, Color.orange, true);
		comp2D.setPaint(gp);
		...
		fl.closePath();
		comp2D.fill(fl);
		// DDraws 3 ovals to make a Mickey Mouse shape.  
